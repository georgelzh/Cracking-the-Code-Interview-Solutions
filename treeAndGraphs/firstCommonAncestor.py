...
1. we go to the first node's parent node,
2. then check if right node's parent node equal to the first. if yes return, else: loop until the root. 
3. if first common ancestor is not found,  then we come back to the first node's parent's parent node.
4. repeat the process 1,2,3 until both first and second node reach its root node. if there is still none found, return False.
...
def find_common_ancestor(root, n1, n2):
    ...
    leftResult = None
    if root.left != None:
        leftResult = find_common_ancestor(root.left, n1, n2)
        # if leftResult != None:
            # print(leftResult)
            # print(root.value)
            # return leftResult
    ...
    rightResult = None
    if root.right != None:
        rightResult = find_common_ancestor(root.right, n1, n2)
        # if rightResult != None:
            # print(rightResult)
            # print(root.value)
            # return rightResult
...
class TestFindAncestor(unittest.TestCase):
    ...
    def test_find_common_ancestor(self):
        ...
        find_common_ancestor(root, 200, 22)
        self.assertEqual(find_common_ancestor(root, 200, 22), 1)
        self.assertEqual(find_common_ancestor(root, 200, 5), 10)
        self.assertEqual(find_common_ancestor(root, 5, 3), 120)
